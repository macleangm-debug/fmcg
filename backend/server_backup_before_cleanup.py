from fastapi import FastAPI, APIRouter, HTTPException, Depends, status, Request, UploadFile, File, Body, WebSocket, WebSocketDisconnect, Header
...
async def get_exchange_rate(currency: str) -> float:
    ...
    if cached:
        ...
        if datetime.utcnow() - last_updated < timedelta(hours=EXCHANGE_RATE_CONFIG["update_interval_hours"]):
            return cached.get("rate", 1)
    
    # Return fallback rate from config
    return EXCHANGE_RATE_CONFIG["fallback_rates"].get(currency, 1)
...
# Re-mount to include kwikpay routes
app.include_router(api_router)

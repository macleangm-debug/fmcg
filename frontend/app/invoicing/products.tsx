...
import { invoiceProductsApi, categoriesApi } from '../../src/api/client';
...
export default function ProductManagement() {
  ...
  const fetchData = async (reset: boolean = true) => {
    try {
      ...
      const [productsRes, categoriesRes] = await Promise.all([
        invoiceProductsApi.getAll({ skip, limit: PAGE_SIZE }),
  ...
  const handleSaveProduct = async () => {
    ...
    try {
      ...
      if (editingProduct) {
        await invoiceProductsApi.update(editingProduct.id, productData);
        setSuccessMessage({
          title: 'Product Updated!',
          subtitle: `"${formName}" has been updated successfully`
        });
      } else {
        await invoiceProductsApi.create(productData);
        setSuccessMessage({
          title: 'Product Added!',
  ...
  const executeDelete = async () => {
    ...
    setDeleting(true);
    try {
      await invoiceProductsApi.delete(productToDelete.id);
  ...
  const renderProductTable = ({ item }: { item: Product }) => {
    const isIncomplete = !item.description || item.price === 0;
    const missingFields = [];
    if (!item.description) missingFields.push('description');
    if (item.price === 0) missingFields.push('price');
    ...
    return (
      <Pressable
        style={[styles.tableRow, isIncomplete && styles.tableRowIncomplete]}
        onPress={() => handleEditProduct(item)}
      >
        <View style={styles.tableCell}>
          {item.image ? (
            ...
            <View style={[styles.tableImagePlaceholder, isIncomplete && styles.imagePlaceholderIncomplete]}>
              <Ionicons name="cube-outline" size={16} color={isIncomplete ? "#F59E0B" : "#9CA3AF"} />
            </View>
          )}
        </View>
        <View style={[styles.tableCell, styles.tableCellName, { flexDirection: 'column', alignItems: 'flex-start' }]}>
          <Text numberOfLines={1} style={{ fontSize: 14, color: '#111827' }}>{item.name}</Text>
          {isIncomplete && (
        ...
        <Text style={[styles.tableCell, styles.tableCellPrice, item.price === 0 && styles.missingText]}>
          {item.price === 0 ? 'Not set' : formatCurrency(item.price)}
        </Text>
        <Text style={[styles.tableCell, styles.tableCellSku]}>{item.sku || '-'}</Text>
        <View style={[styles.tableCell, styles.tableCellActions]}>
          {isIncomplete && (
            <Pressable
              style={styles.completeBtn}
              onPress={(e) => handleActionPress(e, () => handleEditProduct(item))}
            >
              <Ionicons name="create" size={14} color="#FFFFFF" />
            </Pressable>
          )}
          <Pressable
            style={({ pressed }) => [styles.tableActionButton, pressed && styles.actionButtonPressed]}
            onPress={(e) => handleActionPress(e, () => handleEditProduct(item))}
          >
          ...
          <Pressable
            style={({ pressed }) => [styles.tableActionButton, pressed && styles.actionButtonPressed]}
            onPress={(e) => handleActionPress(e, () => handleDeleteProduct(item))}
          >
    ...
    <SafeAreaView style={styles.container}>
      ...
      {isWeb && (
        <View style={styles.webPageHeader}>
          <View>
            <Text style={styles.webPageTitle}>Products & Services</Text>
            ...
            <View style={styles.webStatsRow}>
              ...
              <View style={styles.webStatItem}>
                <Text style={[styles.webStatValue, { color: '#10B981' }]}>
                  {formatNumber(products.filter(p => p.is_active).length)}
                </Text>
                <Text style={styles.webStatLabel}>Active</Text>
              </View>
              <View style={styles.webStatItem}>
      ...
      <WebModal
        ...
        icon={formType === 'service' ? 'construct-outline' : 'bag-add-outline'}
        iconColor="#7C3AED"
        maxWidth={550}
      >
        ...
        <View style={isWeb ? styles.row : styles.mobileColumn}>
          <View style={isWeb ? styles.halfField : styles.fullField}>
            ...
            {globalSkuAutoGenerate ? (
              ...
              <Input
                placeholder={formType === 'service' ? 'SVC-001' : 'PROD-001'}
                value={formSku}
        ...
        <Button
          title={editingProduct ? (formType === 'service' ? 'Update Service' : 'Update Product') : (formType === 'service' ? 'Add Service' : 'Add Product')}

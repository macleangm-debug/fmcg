...
export default function InvoicingSettings() {
  ...
  const [paymentDetails, setPaymentDetails] = useState('Bank: Example Bank\nAccount: 1234567890\nSwift: EXAMPLEXXX');
  ...
  const [invoiceNumberFormat, setInvoiceNumberFormat] = useState('prefix-number'); // prefix-number, prefix-year-number, prefix-yearmonth-number, custom
  const [invoiceStartNumber, setInvoiceStartNumber] = useState('1');
  const [invoiceDigits, setInvoiceDigits] = useState('4'); // padding zeros
  const [invoiceResetYearly, setInvoiceResetYearly] = useState(false);
  const [invoiceSeparator, setInvoiceSeparator] = useState('-');

  // SKU (Product Code) settings
  const [skuPrefix, setSkuPrefix] = useState('PRD');
  const [skuNumberFormat, setSkuNumberFormat] = useState('prefix-number');
  ...
  const [serviceCodeFormat, setServiceCodeFormat] = useState('prefix-number');
  ...
  const INVOICE_FORMATS = [
    { 
      ...
      description: 'Basic format with prefix and number',
      example: 'INV-0001'
    },
    { 
      id: 'prefix-year-number', 
      name: 'Year Based', 
      description: 'Includes year for easy tracking',
      example: 'INV-2025-0001'
    },
    { 
      id: 'prefix-yearmonth-number', 
      name: 'Year-Month Based', 
      description: 'Includes year and month',
      example: 'INV-202501-0001'
    },
    { 
      id: 'year-prefix-number', 
      name: 'Year First', 
      description: 'Year at the beginning',
      example: '2025-INV-0001'
    },
    { 
      id: 'custom', 
      name: 'Custom Format', 
      description: 'Define your own pattern',
      example: 'ACME/2025/0001'
    },
  ];
  ...
  const SKU_FORMATS = [
    ...
    { 
      ...
      name: 'Category Based', 
      description: 'Includes category code',
      example: 'PRD-ELEC-0001'
    },
    { 
      id: 'category-number', 
      name: 'Category Only', 
      description: 'Just category and number',
      example: 'ELEC-0001'
    },
  ];
  ...
  const SERVICE_CODE_FORMATS = [
    ...
    { 
      ...
      name: 'Type Based', 
      description: 'Includes service type (HR=hour, DY=day)',
      example: 'SVC-HR-0001'
    },
    { 
      id: 'type-number', 
      name: 'Type Only', 
      description: 'Just type and number',
      example: 'HR-0001'
    },
  ];
  ...
  const getInvoicePreview = () => {
    ...
    const num = String(parseInt(invoiceStartNumber) || 1).padStart(parseInt(invoiceDigits) || 4, '0');
    const sep = invoiceSeparator;
    ...
    switch (invoiceNumberFormat) {
      ...
      case 'prefix-yearmonth-number':
        return `${prefix}${sep}${year}${month}${sep}${num}`;
      case 'year-prefix-number':
        return `${year}${sep}${prefix}${sep}${num}`;
      case 'custom':
        return `${prefix}${sep}${year}${sep}${num}`;
      default:
        return `${prefix}${sep}${num}`;
  ...
  const getSkuPreview = () => {
    const num = String(parseInt(skuStartNumber) || 1).padStart(parseInt(skuDigits) || 4, '0');
    const sep = skuSeparator;
    const prefix = skuPrefix || 'PRD';
    const categoryCode = 'ELEC'; // Example category

    switch (skuNumberFormat) {
      case 'prefix-number':
        return `${prefix}${sep}${num}`;
      case 'prefix-category-number':
        return `${prefix}${sep}${categoryCode}${sep}${num}`;
      case 'category-number':
        return `${categoryCode}${sep}${num}`;
      default:
        return `${prefix}${sep}${num}`;
    }
  };

  // Generate preview Service Code
  const getServiceCodePreview = () => {
    const num = String(parseInt(serviceCodeStartNumber) || 1).padStart(parseInt(serviceCodeDigits) || 4, '0');
    const sep = serviceCodeSeparator;
    const prefix = serviceCodePrefix || 'SVC';
    const typeCode = 'HR'; // Example type (hourly)

    switch (serviceCodeFormat) {
      case 'prefix-number':
        return `${prefix}${sep}${num}`;
      case 'prefix-type-number':
        return `${prefix}${sep}${typeCode}${sep}${num}`;
      case 'type-number':
        return `${typeCode}${sep}${num}`;
      default:
        return `${prefix}${sep}${num}`;
    }
  };

  // Generic function to check if an app is linked
  const isAppLinked = (appId: string): boolean => {
    if (!currentPlan || !currentPlan.linked_apps || !Array.isArray(currentPlan.linked_apps) || currentPlan.linked_apps.length === 0) {
      return false;
    }
  ...
  // Get linked app info
  const getLinkedAppInfo = (appId: string) => {
    if (!currentPlan?.linked_apps) return null;
    return currentPlan.linked_apps.find((app: any) => 
      app.app_id === appId || app.app_name?.toLowerCase() === appId
    );
  };

  // Check if Inventory app is linked as an add-on
  const isInventoryLinked = (): boolean => isAppLinked('inventory');
  
  // Check if Unitxt app is linked
  const isUnitxtLinked = (): boolean => isAppLinked('unitxt');
  
  // Check if Accounting app is linked
  const isAccountingLinked = (): boolean => isAppLinked('accounting');

  // Check if this app is linked to RetailPro (inherits settings from RetailPro)
  const isLinkedToRetailPro = (): boolean => {
    // If we're in Invoicing and it's a linked app under RetailPro, settings are inherited
    return currentPlan?.primary_app?.id === 'retailpro';
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      {isOnboarding && (
        <View style={styles.onboardingBanner}>
          ...
          <View style={styles.onboardingTextContainer}>
            <Text style={styles.onboardingTitle}>Welcome to Invoicing! 🎉</Text>
      ...
      {!isWeb ? (
        <View style={styles.mobileCardContainer}>
          ...
          {activeTab === 'app' && (
            <View>
              {/* Tax Settings - Inherited from RetailPro when linked */}
              {isLinkedToRetailPro() ? (
                <View style={styles.sectionCard}>
                  <View style={styles.inheritedSettingsHeader}>
                    <Ionicons name="link-outline" size={20} color="#2563EB" />
                    <Text style={styles.inheritedSettingsTitle}>Tax Settings</Text>
                  ...
                  <Text style={styles.inheritedDescription}>
                    Tax settings are managed by your primary RetailPro app. Changes made in RetailPro will automatically apply to invoices.
                  </Text>
                  
                  <View style={styles.inheritedSettingsBox}>
                    <View style={styles.inheritedSettingRow}>
                      <Text style={styles.inheritedSettingLabel}>Apply Tax</Text>
                      <Text style={styles.inheritedSettingValue}>{applyTax ? 'Yes' : 'No'}</Text>
                    </View>
                    {applyTax && (
                      <>
                        <View style={styles.inheritedSettingRow}>
                          <Text style={styles.inheritedSettingLabel}>Default Tax Rate</Text>
                          <Text style={styles.inheritedSettingValue}>{defaultTaxRate || '0'}%</Text>
                        </View>
                        <View style={styles.inheritedSettingRow}>
                          <Text style={styles.inheritedSettingLabel}>Prices Tax Inclusive</Text>
                          <Text style={styles.inheritedSettingValue}>{taxInclusive ? 'Yes' : 'No'}</Text>
                        </View>
                      </>
                    )}
                  </View>
                ...
                <View style={styles.sectionCard}>
                  ...
                  <View style={styles.settingRow}>
                    <View style={styles.settingInfo}>
                      ...
                      <Text style={styles.settingDescription}>Enable tax on invoices</Text>
                    </View>
                    <Switch
                      value={applyTax}
              ...
              <View style={styles.sectionCard}>
                ...
                <View style={styles.paymentDetailsContainer}>
                  ...
                  <Text style={styles.paymentDetailsHint}>
                    This will be displayed on the bottom left of the invoice
                  </Text>
                  <TextInput
                    style={styles.paymentDetailsInput}
                    placeholder="Enter your payment details (Bank name, Account number, Swift code, etc.)"
                    placeholderTextColor="#9CA3AF"
              ...
              <View style={styles.infoCard}>
                ...
                <View style={styles.infoCardContent}>
                  ...
                  <Text style={styles.infoCardText}>
                    Link the Inventory app to manage stock, get low stock alerts, and auto-deduct inventory on invoices.
                  </Text>
                  <TouchableOpacity 
              ...
              <Button
                title="Save"
                onPress={() => {
                  Alert.alert('Settings Saved', 'Your Invoicing settings have been saved successfully.');
                }}
                loading={saving}
                style={styles.saveButton}
              />
                    ...
                    <View>
                      ...
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                        <Text style={styles.currentPlanName}>
                          {currentPlan?.primary_app?.name || 'Invoicing'} {currentPlan?.plan?.name || 'Starter'}
                        </Text>
            ...
            {activeTab === 'app' && (
              <View>
                ...
                <View style={styles.sectionCard}>
                  ...
                  <Text style={styles.sectionSubtitle}>Customize how your invoice numbers are generated</Text>
                  
                  {/* Format Selection */}
                  <Text style={styles.formatSectionLabel}>Number Format</Text>
                  <View style={styles.formatOptionsGridWeb}>
                    {INVOICE_FORMATS.map((format) => (
                      <TouchableOpacity
                        key={format.id}
                        style={[
                          styles.invoiceFormatOption,
                          invoiceNumberFormat === format.id && styles.invoiceFormatOptionActive
                        ]}
                        onPress={() => setInvoiceNumberFormat(format.id)}
                      >
                        <View style={styles.formatOptionHeader}>
                          <View style={[
                            styles.formatRadio,
                            invoiceNumberFormat === format.id && styles.formatRadioActive
                          ]}>
                            {invoiceNumberFormat === format.id && (
                              <View style={styles.formatRadioInner} />
                            )}
                          </View>
                          <Text style={[
                            styles.formatOptionTitle,
                            invoiceNumberFormat === format.id && styles.formatOptionTitleActive
                          ]}>
                            {format.name}
                          </Text>
                        </View>
                  ...
                  <View style={styles.inputRow}>
                    ...
                    <View style={styles.inputHalf}>
                      <Input
                        ...
                        onChangeText={setInvoiceStartNumber}
                          ...
                          {SEPARATOR_OPTIONS.map((sep) => (
                            <TouchableOpacity
                              ...
                              style={[
                                ...
                                invoiceSeparator === sep.value && styles.separatorOptionActive
                              ]}
                              onPress={() => setInvoiceSeparator(sep.value)}
                            >
                              <Text style={[
                                styles.separatorOptionText,
                                invoiceSeparator === sep.value && styles.separatorOptionTextActive
                              ]}>
                                {sep.label}
                              </Text>
                            </TouchableOpacity>
                  ...
                  <View style={styles.settingRow}>
                    <View style={styles.settingInfo}>
                      <Text style={styles.settingLabel}>Reset Yearly</Text>
                      <Text style={styles.settingDescription}>Start from 1 at the beginning of each year</Text>
                    </View>
                    <Switch
                      value={invoiceResetYearly}
                      onValueChange={setInvoiceResetYearly}
                      trackColor={{ false: '#E5E7EB', true: '#BFDBFE' }}
                      thumbColor={invoiceResetYearly ? '#2563EB' : '#9CA3AF'}
                    />
                  </View>

                  {/* Live Preview */}
                  <View style={styles.invoicePreviewBox}>
                    <Text style={styles.invoicePreviewLabel}>Next Invoice Number Preview</Text>
                    <Text style={styles.invoicePreviewValue}>{getInvoicePreview()}</Text>
                  </View>
                </View>

                {/* SKU Formatting Section */}
                <View style={styles.sectionCard}>
                  <Text style={styles.sectionTitle}>Product SKU Format</Text>
                  <Text style={styles.sectionSubtitle}>Customize how product SKUs are auto-generated</Text>
                  ...
                  <View style={styles.formatOptionsGridWeb}>
                    {SKU_FORMATS.map((format) => (
                      <TouchableOpacity
                        key={format.id}
                        style={[
                          styles.skuFormatOption,
                          skuNumberFormat === format.id && styles.skuFormatOptionActive
                        ]}
                        onPress={() => setSkuNumberFormat(format.id)}
                      >
                        <View style={styles.formatOptionHeader}>
                          <View style={[
                            styles.formatRadio,
                            skuNumberFormat === format.id && styles.skuFormatRadioActive
                          ]}>
                            {skuNumberFormat === format.id && (
                              <View style={styles.formatRadioInner} />
                            )}
                          </View>
                          <Text style={[
                            styles.formatOptionTitle,
                            skuNumberFormat === format.id && styles.skuFormatOptionTitleActive
                          ]}>
                            {format.name}
                          </Text>
                        </View>
                        ...
                        <View style={styles.separatorOptionsRow}>
                          {SEPARATOR_OPTIONS.map((sep) => (
                            <TouchableOpacity
                              ...
                              style={[
                                styles.separatorOption,
                                skuSeparator === sep.value && styles.skuSeparatorOptionActive
                              ]}
                              onPress={() => setSkuSeparator(sep.value)}
                            >
                              <Text style={[
                                styles.separatorOptionText,
                                skuSeparator === sep.value && styles.skuSeparatorOptionTextActive
                              ]}>
                                {sep.label}
                              </Text>
                            </TouchableOpacity>
                ...
                {/* Service Code Formatting Section */}
                <View style={styles.sectionCard}>
                  <Text style={styles.sectionTitle}>Service Code Format</Text>
                  <Text style={styles.sectionSubtitle}>Customize how service codes are auto-generated</Text>
                  ...
                  <View style={styles.formatOptionsGridWeb}>
                    {SERVICE_CODE_FORMATS.map((format) => (
                      <TouchableOpacity
                        key={format.id}
                        style={[
                          styles.serviceFormatOption,
                          serviceCodeFormat === format.id && styles.serviceFormatOptionActive
                        ]}
                      ...
                      >
                        <View style={styles.formatOptionHeader}>
                          <View style={[
                            styles.formatRadio,
                            serviceCodeFormat === format.id && styles.serviceFormatRadioActive
                          ]}>
                            {serviceCodeFormat === format.id && (
                              <View style={styles.formatRadioInner} />
                            )}
                          </View>
                          <Text style={[
                            styles.formatOptionTitle,
                            serviceCodeFormat === format.id && styles.serviceFormatOptionTitleActive
                          ]}>
                            {format.name}
                          </Text>
                        </View>
                  ...
                  <View style={styles.servicePreviewBox}>
                    <Text style={styles.servicePreviewLabel}>Next Service Code Preview</Text>
                    <Text style={styles.servicePreviewValue}>{getServiceCodePreview()}</Text>
                  </View>
                </View>
        ...
        <View style={styles.successModalOverlay}>
          <View style={styles.successModalContent}>
            ...
            <Text style={styles.successModalMessage}>
              {isOnboarding 
                ? 'Your invoicing business is ready. Start creating invoices!'
                : showSuccessModal.tab === 'business'
                  ? 'Business settings saved successfully'
                  : showSuccessModal.tab === 'invoicing'
                    ? 'Invoicing settings saved successfully'
...
const styles = StyleSheet.create({
  ...
  // Invoice format option - matching SKU style
  invoiceFormatOption: {
    flex: 1,
    minWidth: 200,
    backgroundColor: '#F9FAFB',
  ...
  skuFormatRadioActive: {
    borderColor: '#2563EB',
  },
  skuFormatOptionTitleActive: {
    color: '#1D4ED8',
  },
  skuSeparatorOptionActive: {
    backgroundColor: '#2563EB',
    borderColor: '#2563EB',
  },
  skuSeparatorOptionTextActive: {
    color: '#FFFFFF',
  },
  skuPreviewBox: {
    backgroundColor: '#EFF6FF',
  ...
  serviceFormatOptionActive: {
    borderColor: '#10B981',
    backgroundColor: '#ECFDF5',
  },
  serviceFormatRadioActive: {
    borderColor: '#10B981',
  },
  serviceFormatOptionTitleActive: {
    color: '#047857',
  },
  ...
  // Section Header with Add-on Badge
  sectionHeaderRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'flex-start',
  ...
  linkInventoryFeatures: {
    alignSelf: 'stretch',
    gap: 10,
    marginBottom: 20,
  },
  linkInventoryFeature: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
  },
  linkInventoryFeatureText: {
    fontSize: 14,
    color: '#374151',
  },
  linkInventoryBtn: {
